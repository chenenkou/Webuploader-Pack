window.BaiduUpload = (function ($, WebUploader) {
    function BaiduUpload(options, setting) {
        ...
        var that = this;
        this.setting = $.extend({
            fileQueued: that._defaultFileQueued, // 加入队列
            error: that._defaultError, // 验证
            uploadProgress: that._defaultUploadProgress, // 上传中
            uploadComplete: that._defaultUploadComplete // 完成
        }, setting);
    ...
    // 默认方法 加入队列-验证-上传中-完成
    BaiduUpload.prototype._defaultFileQueued = function (that, file) {
        var $uploader = $(that.uploader);
        var $statusBar = $uploader.find(".statusBar");
        $statusBar.html('\
                    <div class="progress"><span class="text"></span><span class="percentage"></span></div>\
                ');
    };
    BaiduUpload.prototype._defaultError = function (type) {
        switch (type) {
            case "F_EXCEED_SIZE":
                alert('文件大小超过限制');
                break;
    ...
    BaiduUpload.prototype._defaultUploadProgress = function (that, file, percentage) {
        var $uploader = $(that.uploader),
    ...
    BaiduUpload.prototype._defaultUploadComplete = function (that, file) {
        var $uploader = $(that.uploader);
        var $statusBar = $uploader.find(".statusBar");
        setTimeout(function () {
            $statusBar.html("");
    ...
    BaiduUpload.prototype.fileQueued = function (callback) {
        if ($.isFunction(callback)) {
            this.setting.fileQueued = callback;
            return true;
        }
        var that = this;
    ...
    // 文件验证不通过
    BaiduUpload.prototype.error = function (callback) {
        if ($.isFunction(callback)) {
            this.setting.error = callback;
            return true;
        }
        var that = this;
        this.handler.on('error', function (type) {
            that.setting.error(type);
        });
    };
    ...
    BaiduUpload.prototype.uploadProgress = function (callback) {
        if ($.isFunction(callback)) {
            this.setting.uploadProgress = callback;
            return true;
        }
        var that = this;
        this.handler.on('uploadProgress', function (file, percentage) {
            that.setting.uploadProgress(that, file, percentage);
        });
    };
    ...
    BaiduUpload.prototype.uploadComplete = function (callback) {
        if ($.isFunction(callback)) {
            this.setting.uploadComplete = callback;
            return true;
        }
        var that = this;
        this.handler.on('uploadComplete', function (file) {
            that.setting.uploadComplete(that, file);
        });
    };
